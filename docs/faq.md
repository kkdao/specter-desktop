...
We named it after a vulnerability, since we learned that everything can be hacked! Although, when you combine different technical systems in an user-friendly multisig setup you can significantly increase the security of your cold storage. 


### GENERAL QUESTIONS


*How safe is the app to use?  Is it still considered alpha/beta or safe enough to use it with real sats in a Colcard multisig setup?*
...
Specter-desktop is only a wrapper (watch-only) around Bitcoin Core wallet functionality, mostly, which uses your full node. Bitcoin Core tracks addresses, UTXO (unspent transaction outputs) and composes PSBT (partially-signed bitcoin transactions). But all wallets are watch-only.
...
*Just wanted to know if you need Specter to have a wallet on your node for it to even start? I was hoping to try this without using the core wallet at all.*

You don't need private keys in Bitcoin Core, but you need wallets to be enabled in your bitcoin.conf file `disablewallet=0`.
...
Yes, this is the plan - to use a HW wallet like Coldcard/Trezor with Specter DIY, with a user-friendly multisig Specter desktop app, which is connected to your own node for better privacy.
...
It works better with hardware wallets: https://github.com/bitcoin-core/HWI  
...
First you need to “add devices” that store keys for the wallet. Once you add at least two devices, then the multsig option will become available to you in the UI.


*Can I use Bluewallet with Specter DIY?*

Yes you can use BlueWallet in watch-only mode and sign with Specter DIY
...
Absolutely, as well as any other DIY bitcoin full node, or even pruned node!
...
The URL tab can be left blank for now. It is a feature currently being improved upon **(???)**

**Note: Last I used this was the case, perhaps it's changed now?**
...
Yes, it's a standard multisig. So you can recreate it as soon as you have **master public keys of the devices** - either with Specter, or Electrum. When using Specter and importing old wallet you would need to rescan blockchain in the wallet settings page.
...
See [shopping list](https://github.com/cryptoadvance/specter-diy/blob/master/docs/shopping.md) and [assembly](https://github.com/cryptoadvance/specter-diy/blob/master/docs/assembly.md)
...
Putting it into HWI means: "this is a hardware wallet people should consider using for real". Currently, we would strongly advise NOT to use USB with specter-DIY, but to use QR codes instead. We will make a PR to HWI when we think it's safe enough. In particular when we will have a secure bootloader that verifies signatures of the firmware, and USB communication is more reliable.
...
Yes. Specter-DIY in simulator-mode simulates QR code scanner over TCP, see link [here](https://diybitcoinhardware.com/f469-disco/simulator/?script=https://raw.githubusercontent.com/diybitcoinhardware/f469-disco/master/docs/tutorial/4_miniwallet/main.py)
...
*Keep getting error: No matching distribution found for cryptoadvance.specter*

*I'm still running into one issue, when I try to run specter with:*
`$ python -m cryptoadvance.specter server`

*It states 'no module named cryptoadvance'*
...
*How to delete a wallet using a remote node?*
...
Use as small a powerbank as possible.
...
Right now it’s only with external wallets, but there’s an open issue for hot wallet. We have plans to add new device type "this computer", see issue [here](https://github.com/cryptoadvance/specter-desktop/issues/58)
...
I didn't play with it yet. We will work on the bootloader soon and try different update mechanisms. QR codes is one of them. Also considering SD card - might be easier as firmware is 1Mb, so it would require 1000 QR codes.
...
*What is the difference between DIYbitcoinhardware & Specter? Is diybitcoinhardware sort of a prerequisite for Specter?*
